...
public class DesignerHeightLayoutCache extends AbstractLayoutCache {
    /**
    ...
    public DesignerHeightLayoutCache() {
		super();
		
		System.out.println( "DesignerHeightLayoutCache()");
	...
	public int getChildCount( Object value) {
		...
		if ( value instanceof DesignerNode) {
			DesignerNode node = (DesignerNode)value;
			Enumeration nodes = node.children();

			while ( nodes.hasMoreElements()) {
				DesignerNode child = (DesignerNode)nodes.nextElement();
	...
	public int getChildCount() {
		...
		while ( nodes.hasMoreElements()) {
			TreeStateNode child = (TreeStateNode)nodes.nextElement();
			Object value = child.getValue();
	...
	public TreeNode getChild( Object value, int index) {
		int children = 0;
		
		if ( value instanceof DesignerNode) {
			Enumeration nodes = ((DesignerNode)value).children();
		...
		System.out.println( "getChild( "+value+", "+index+") [null]");
	...
	public int getIndex( TreeNode node) {
		...
		while ( nodes.hasMoreElements()) {
			...
			if ( value instanceof DesignerNode && !((DesignerNode)value).isVirtual()) {
				children++;
			}
		}
	...
	public TreeNode getChildAt( int index) {
		...
		System.out.println( "getChildAt( "+index+") [null]");
...
//		}
	...
	protected Enumeration getLoadedChildren(boolean createIfNeeded) {
	    ...
	    int             count = getChildCount(realNode);
	    ...
	    if(childRow == -1) {
			for (int i = 0; i < count; i++) {
			    newNode = createNodeForValue( getChild( realNode, i));
			    this.add(newNode);
