...
<xsl:choose>
    ...
    <xsl:choose>
    	<xsl:when test="$where">
    		<sql:query connection="$connection" table="{$table}" column="{$column}" where="{$where}" row-tag="row" column-tag="col" xsl:extension-element-prefixes="sql"/>
    	</xsl:when>
    	<xsl:otherwise>
     		<sql:query connection="$connection" table="{$table}" column="{$column}" row-tag="row" column-tag="col" xsl:extension-element-prefixes="sql"/>   	
    	</xsl:otherwise>
    </xsl:choose> 
