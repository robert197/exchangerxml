  /*
 * $Id: WSDLDialog.java,v 1.13 2004/10/29 15:01:32 tcurley Exp $
 *
...
import com.cladonia.xml.webservice.soap.XMLPane;
 ...
 * @author Dogsbay
 */
public class WSDLDialog extends XngrDialogHeader implements ActionListener {
...
//	private JCheckBox openAsNewDocumentBox	= null;
	...
	private JComboBox servicesField = null;
	private JComboBox operationsField = null;
	 ...
	 */	
	public WSDLDialog( ExchangerEditor parent, ConfigurationProperties props) {
		super( parent, false);
		
		setResizable( true);
		setTitle( "Analyze WSDL Document");
		setDialogDescription( "Specify WSDL information.");
		...
		JPanel wsdlPanel = new JPanel( new FormLayout( 10, 2));
		wsdlPanel.setBorder( new CompoundBorder( 
									new TitledBorder( "WSDL"),
									new EmptyBorder( 0, 5, 5, 5)));

		servicesField = new JComboBox();
		servicesField.setEditable( false);
		servicesField.setPreferredSize( new Dimension( 100, 21));
		servicesField.setSelectedItem( null);
//		servicesField.addActionListener( new ActionListener(){
//			public void actionPerformed( ActionEvent e) {
//				System.out.println( "servicesField.actionPerformed()");
//				ServiceInfo service = (ServiceInfo)servicesField.getSelectedItem();
//				
//				if ( service != null) {
//					setOperations( service.getOperations());
//				} else {
//					setOperations( null);
//				}
//			}
//		});
		servicesField.addItemListener( new ItemListener(){
			public void itemStateChanged( ItemEvent e) {
//				System.out.println( "servicesField.itemStateChanged()");
				Object service = servicesField.getSelectedItem();
				
				if ( service != null) {
					setOperations( ((ServiceInfo)service).getOperations());
				} else {
					setOperations( null);
				}
			}
		...
		operationsField = new JComboBox( new Vector( properties.getActions()));
		operationsField.setEditable( false);
		operationsField.setPreferredSize( new Dimension( 100, 21));
		operationsField.setSelectedItem( null);
		operationsField.addItemListener( new ItemListener(){
			public void itemStateChanged( ItemEvent e) {
//				System.out.println( "operationsField.itemStateChanged()");
				
				Object object = operationsField.getSelectedItem();
				
				if ( object != null) {
					setOperation( (OperationInfo)object);
				} else {
					setOperation( null);
				}
			}
		...
		wsdlPanel.add( new JLabel( "Services:"), FormLayout.LEFT);
		wsdlPanel.add( servicesField, FormLayout.RIGHT_FILL);
		
		wsdlPanel.add( new JLabel( "Operations:"), FormLayout.LEFT);
		wsdlPanel.add( operationsField, FormLayout.RIGHT_FILL);
...
//		getRootPane().setDefaultButton( sendButton);
		...
		formPanel.add( wsdlPanel, FormLayout.FULL_FILL);
...
//		mainPanel.add( buttonPanel, BorderLayout.SOUTH);
	...
	public void show( ExchangerDocument document) { // SOAPException, SAXParseException, IOException {
		...
		if ( url != null) {
			...
			setTitle( "WSDL Analyser ["+location+"]","Inspect WSDL Document");
		} else {
			setTitle( "WSDL Analyser ["+document.getName()+"]","Inspect WSDL Document");
		}

		try {
			setServices( parser.getServicesInfo( baseURI, document.getW3CDocument()));

			//super.setVisible(true);
			super.show();
		} catch ( Exception e) {
			e.printStackTrace();
			MessageHandler.showError( "Error creating the WSDL message.", e.getCause(), "WSDL Error");
		}
	}
	
	private void setServices( List services) {
		servicesField.removeAllItems();
		...
		if ( services != null) {
			...
			while ( servicesIter.hasNext()) {
				...
				servicesField.addItem( service);
			}
			
			servicesField.setSelectedIndex( 0);

			setOperations( ((ServiceInfo)servicesField.getSelectedItem()).getOperations());
		} else {
			setOperations( null);
		}
	}
	
	private void setOperations( Iterator operations) {
		operationsField.removeAllItems();
		
		if ( operations != null) {

			while ( operations.hasNext()) {
				OperationInfo operation = (OperationInfo)operations.next();
				operationsField.addItem( operation);
			}
			
			operationsField.setSelectedIndex( 0);
			setOperation( (OperationInfo)operationsField.getSelectedItem());
		} else {
			setOperation( null);
		}
	}

	private void setOperation( OperationInfo operation) {
		if ( operation != null) {
			initEditor( urlField, operation.getTargetURL());
			initEditor( actionField, operation.getSoapActionURI());

			setSOAPMessage( operation.getInputMessageText());
		} else {
			initEditor( urlField, "http://");
			initEditor( actionField, "");
	...
	public void setSOAPMessage( String text) {
...
//		  *******************START new code*********************************
	...
	private void setMessage() { // SOAPException, SAXParseException, IOException {
		try{
			...
			if ( inputPane.getText() != null && inputPane.getText().trim().length() > 0) {
...
//		System.out.println( "SOAPDialog.send( "+target+", "+action+")");
		...
		setMessage();
		final AutomaticProgressMonitor monitor = new AutomaticProgressMonitor( parent, null, "Sending SOAP message ...", 200);
