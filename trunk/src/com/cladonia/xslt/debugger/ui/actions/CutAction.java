/*
 * $Id: CutAction.java,v 1.1 2005/08/26 11:03:41 tcurley Exp $
 *
...
/**
 * An action that can be used to cut
 * Xml Editor.
 *
...
public class CutAction extends AbstractAction {
	 ...
	 */
	public CutAction( JFrame parent, XSLTDebuggerPane debugger) {
		super( "Cut");
	...
	public void actionPerformed( ActionEvent event) {
		if (DEBUG) System.out.println( "CutAction.actionPerformed( "+event+")");
		...
		if (( pane != null) || (outPane != null) ) {
			if(isInputPane) {
				...
				if ( pane != null) {
					((XmlEditorPane)pane.getEditor()).cut();
				}	
			}
			else {
				...
				if ( outPane != null) {
					...
					if(editorObj instanceof XmlEditorPane) {
						((XmlEditorPane)editorObj).cut();
					}
					else if(editorObj instanceof JTextArea) {
						
						((JTextArea)editorObj).cut();
					}
					else {
