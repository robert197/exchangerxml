...
public final class BidiUtils {
    ...
    public static void getLevels(Bidi bidi, byte[] levels, int start) {
        ...
        if (start < 0 || limit > levels.length) {
            throw new IndexOutOfBoundsException("levels.length = "
                    + levels.length + " start: " + start + " limit: "
                    + limit);
        }
     ...
     */
    private static int[] computeContiguousOrder(int[] values,
            int start, int limit) {
     ...
     */
    public static int[] createNormalizedMap(int[] values,
            byte[] levels, int start, int limit) {
        ...
        if (values != null) {
            if (start != 0 || limit != values.length) {
                ...
                if (copyRange) {
                    ...
                    if (baseValue == 0) {
                        System.arraycopy(values, start, result, 0,
                                limit - start);
                    } else {
