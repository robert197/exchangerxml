<!-- 
TxView.xsl
XBRL Version 2 Taxonomy Viewer Transformation

This XSLT code, when applied to a version 2 XBRL taxonomy schema 
document, will output a simple HTML table containing various pieces of 
...
Placing the following line in your XBRL taxonomy schema file directly before
the root <schema> element will cause some browsers and processors to 
...
<Disclaimer>
  ...
  produce a basic, summarizing taxonomy viewer.  Some features of XBRL, 
...
This template performs two steps:
1) find the linkbaseRef elements, and process the contents of the
   linkbases that are named (regardless of the xlink:actuate attribute)
2) for each schema element, report its name and type, as well as 
   list various information, such as label(s), definition(s), etc.
   that was gathered in step 1
-->
<xsl:template match="/">
  <!-- step 1: find and resolve linkbaseRef elements -->
  <xsl:variable name="extLinks">
  <xsl:apply-templates select="*" />
  </xsl:variable>

  <!-- step 2: for each element, report its information.  This step 
  makes use of a named template called "schemaOutput"
  -->
  <HR/><H2>XBRL Version 2 Taxonomy Viewer</H2><HR/>
  <xsl:for-each select="//xsd:element">
    <xsl:call-template name="schemaOutput">
...
being reported, and a node-list of information found in the linkbases. It outputs
an HTML table which lists various information and relationships.  
-->
<xsl:template name="schemaOutput"> 
  <xsl:param name="schemaElt" />
  <xsl:param name="extendedLinks" />
  
  <!-- create an HTML table in which to report the various pieces of data -->
  <table >
    <tr><td align="left"><H2><xsl:value-of select="$schemaElt/@name" /></H2></td></tr>
    <tr><th align="left">Type</th><th align="left">Subst. Grp.</th></tr>
      <tr>
      <td ><xsl:value-of select="$schemaElt/@type" /></td>
      <td ><xsl:value-of select="$schemaElt/@substitutionGroup" /></td>
      </tr>
      
      <!-- output labels -->
      <tr height="3" /><td>---</td>
      <tr><th align="left">Labels</th></tr>
      <tr><td><i>Label</i></td><td><i>Lang.</i></td></tr>
      <xsl:for-each select="($extendedLinks)//tempLink:label[substring-after(@tempLink:from, '#') = $schemaElt/@name]">
        <tr><td>&#x20;&#x20;&#x20;&#x20;<xsl:value-of select="."/></td><td><xsl:value-of select="@xml:lang" /></td></tr>
      </xsl:for-each>

      <!-- output calculation relationships -->
      <!-- elements for which the schema element in question is a parent -->
      <tr height="3" /><td>---</td>
      <tr><th  align="left">Calculation Relationships</th></tr>
      <tr><td><i>Child(ren)</i></td></tr>
      <xsl:for-each select="($extendedLinks)//tempLink:calculationArc[substring-after(@tempLink:from, '#') =
        ...
        <tr><td ><xsl:value-of select="@tempLink:to"/></td></tr>
      </xsl:for-each>

      <!-- elements for which the schema element in question is a child -->
      <tr height="3" />
      <tr><td><i>Parent(s)</i></td></tr>
      <xsl:for-each select="($extendedLinks)//tempLink:calculationArc[substring-after(@tempLink:from, '#') = 
                    $schemaElt/@name and @xlink:arcrole = 'http://www.xbrl.org/linkprops/arc/child-parent']">
        <tr><td ><xsl:value-of select="@tempLink:to"/></td></tr>
      </xsl:for-each>

      <!-- output presentation relationships -->
      <!-- elements for which the schema element in question is a parent -->
      <tr height="3" /><td>---</td>
      <tr><th  align="left">Presentation Relationships</th></tr>
      <tr><td><i>Child(ren)</i></td></tr>
      ...
      <!-- output definition relationships -->
      <!-- elements for which the schema element in question is a parent -->
      <tr height="3" /><td>---</td>
      <tr><th  align="left">Definition Relationships</th></tr>
      <tr><td><i>has-a</i></td></tr>
      ...
      <!-- elements for which the schema element in question is a child -->
      <tr height="3" />
      <tr><td><i>is-a</i></td></tr>
      <xsl:for-each select="($extendedLinks)//tempLink:definitionArc[substring-after(@tempLink:from, '#') = 
                    $schemaElt/@name and @xlink:arcrole = 'http://www.xbrl.org/linkprops/arc/child-parent']">
        <tr><td ><xsl:value-of select="@tempLink:to"/></td></tr>
      </xsl:for-each>

      <!-- output reference(s) here, element to reference arcs only -->
      <tr height="3" /><td>---</td>
      <tr><th  align="left">Reference(s) - Text only</th></tr>
